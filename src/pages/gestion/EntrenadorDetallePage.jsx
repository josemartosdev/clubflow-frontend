        ...
        <ul className="gestion-tactics-list">
          {(coach.tactics || []).map((t) => (
            <li key={t.id} className="gestion-tactic-card">
              ...
              {t.formacion && (
                <div className="gestion-tactic-card__pitch-wrap">
                  <FormationPitch formation={t.formacion} />
                </div>
              )}
              {t.diagrama_path && (
          ...
          <form className="gestion-form gestion-form--grid gestion-tactic-form" onSubmit={saveTactic}>
            ...
            <div className="gestion-field gestion-field--full">
              ...
              <div className="formation-picker">
                {FORMATION_OPTIONS.map((code) => (
                  <button
                    key={code}
                    type="button"
                    className={`formation-picker__btn${tacticForm.formacion === code ? " formation-picker__btn--active" : ""}`}
                    onClick={() => setTacticForm({ ...tacticForm, formacion: code })}
                  >
                    {code}
                  </button>
                ))}
              ...
              <input
                ...
                onChange={(e) => setTacticForm({ ...tacticForm, formacion: e.target.value })}
              />
            </div>
            {tacticForm.formacion && (
              <div className="gestion-field--full gestion-tactic-card__pitch-wrap">
                <FormationPitch formation={tacticForm.formacion} compact />
              </div>
            )}
            <label className="gestion-field">
